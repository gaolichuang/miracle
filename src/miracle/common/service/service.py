...
from miracle.common.base import eventlet_backdoor
...
from miracle.common.base import threadgroup
...
service_opts = [
    ...
    cfg.IntOpt('periodic_report_tasks_interval',
               default=60,
               help='default peridic report tasks interval use in manager')
]
...
class ProcessLauncher(object):
    '''this is use for process not thread, use os.fork below'''
    def __init__(self):
        self.children = {}
        self.sigcaught = None
        self.running = True
...
def create():
    pass
''' start the server, worker means process'''
def serve(server, workers=None):
    global _launcher
    if _launcher:
        raise RuntimeError(_('serve() can only be called once'))

    _launcher = launch(server, workers=workers)
